...
def create_siamese_dataset(data):
    ...
    for entry in data:
        ...
        for sens_id, synset in enumerate(entry['synsets']):
            for example in synset['examples']:
                entry_examples.append({"example": example["ex_text"], "sense_id": sens_id, "lemma": entry["lemma"]})
    ...
    for examples in all_examples:
        ...
        for pair in combinations(examples, 2):
            ...
            pairs.append({"sentence1": sen1, "sentence2": sen2, "label": label, "lemma": pair[0]["lemma"]})
