raw_file_path = "data/input_dfs/sum_14_final.jsonlines"
filtered_file_path = "data/final_results/sum14_filtered.json"
merged_file_path = "data/final_results/sum14_merged.json"

lcopf_file_path = 'data/input_dfs/ulif_accents.csv'
lcopf_forms_file_path = 'data/final_results/lcopf_forms.csv'

lcopf_forms_not_found_file_path = 'data/final_results/forms_not_found.jsonl'
lcopf_forms_found_file_path = 'data/final_results/forms_found.jsonl'

pairs_file_path = "data/final_results/sum14_pairs.jsonl"
pairs_forms_file_path = "data/final_results/sum14_pairs_forms.jsonl"

train_file_path = "data/final_results/sum14_pairs-train.jsonl"
test_file_path = "data/final_results/sum14_pairs-test.jsonl"

train_forms_file_path = "data/final_results/sum14_pairs_forms-train.jsonl"
test_forms_file_path =  "data/final_results/sum14_pairs_forms-test.jsonl"

train_lem_file_path = "data/final_results/sum14_pairs_lem-train.jsonl"
train_not_lem_file_path = "data/final_results/sum14_pairs_not_lem-train.jsonl"

test_lem_file_path =  "data/final_results/sum14_pairs_lem-test.jsonl"
test_not_lem_file_path =  "data/final_results/sum14_pairs_not_lem-test.jsonl"

filtered_extra_file_path = "data/final_results/sum14_filtered-extra.json"



MIN_LEMMA_LEN = 3
MIN_GLOSS_LEN = 5
MIN_EXAMPLE_LEN = 10
MIN_EXAMPLE_NUM = 1
MIN_SYNSET_NUM = 2
MIN_SYNSET_NUM_PRIME = 1