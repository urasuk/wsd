...
DATASET_NAMES = [
    "targets_df_train_xlm_separate_masked.jsonl",
    "targets_df_test_xlm_separate_masked.jsonl"
]

FILE_PATH_TRAIN_DF = f"./data/input_dfs/{DATASET_NAMES[0]}"
FILE_PATH_TEST_DF = f"./data/input_dfs/{DATASET_NAMES[1]}"

FILE_PATH_TO_SAVE_MODEL = "./weights/model_xlm_with_tags_sep_mse.pth"
CHECKPOINT_MODEL_PATH = "./weights/model_xlm_with_tags_sep_mse_checkpoint.pth" 
...
MIN_LR = 5e-9
INITIAL_LR = 5e-4 # for contrastive loss!
DIST_METRIC = 'cosine' # euclid + contrastive loss
PATIENCE = 3
FACTOR = 0.2
...
class SiameseNNBatchSepXlm(nn.Module):
    def __init__(self, checkpoint, proj_target_len=PROJ_TARGET_LEN, only_head=False, simple_head=1, use_lora=False):
        # only_head:
        # -1 = all
        # 1 = head
        # n = last 'n' xlm layers + head    (n > 1)
        
        super(SiameseNNBatchSepXlm, self).__init__()
        self.only_head = only_head
    
        self.xlm = AutoModel.from_pretrained(checkpoint)
        
#         if use_lora:
#             # Applying LoRA to the xlm layers
#             lora_config = LoraConfig(
#                 r=1, lora_alpha=1, lora_dropout=0.1, target_modules=["query", "value"]
#             )
#             self.xlm = apply_lora(self.xlm, lora_config)
        
        if simple_head == 1:
            self.head = nn.Sequential(
                nn.Linear(self.xlm.config.hidden_size, proj_target_len),
            ) 
        elif simple_head == 0:
            self.head = nn.Sequential(
                nn.Linear(self.xlm.config.hidden_size, 512),
                nn.LeakyReLU(),
#                 nn.Dropout(0.1),
                nn.Linear(512, 256),
                nn.LeakyReLU(),
#                 nn.Dropout(0.1),
    ...
    def parameters(self, recurse: bool = True):
        if self.only_head == 1:
            ...
            print("All xlm layers + HEAD")
            return super(SiameseNNBatchSepXlm, self).parameters(recurse)
        else:
            first_n_to_freeze = self.only_head
            self.freeze_first_n(first_n_to_freeze)
#             modules = [*self.xlm.encoder.layer[first_n_to_freeze:], self.head]
#             return (module.parameters() for module in modules)
            return super(SiameseNNBatchSepXlm, self).parameters(recurse)

        
    def freeze_first_n(self, n):
        modules = [self.xlm.embeddings, *self.xlm.encoder.layer[:n]]
...
def main():
    ...
    model = SiameseNNBatchSepXlm(checkpoint, simple_head=-1, only_head=-1)
    ...
    #     plot_loss_accuracy(train_losses, train_accuracies, "Train", "xlm (no tags) (sep)")
    #     plot_loss_accuracy(test_losses, test_accuracies, "Test", "xlm (no tags) (sep)")
        # plot_loss_accuracy(inter_valid_losses, inter_valid_accuracies, "Valid inter", "xlm (no tags) (sep)")
